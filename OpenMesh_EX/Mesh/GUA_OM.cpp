...
void Tri_Mesh::loadToBuffer(Tri_Mesh _mesh, std::vector<double> & out_vertices, int & face, std::vector<double> & uv, std::vector<double> & modelCenter) {
	...
	for (f_it = faces_begin(); f_it != faces_end(); ++f_it) {
		...
		for (int i = 0; i < 3; i++, ++fv_it) {
			// 每個點有三個vertexes
			out_vertices.push_back((*(point(fv_it.handle()).data()) - center[0]) / scalar);
			out_vertices.push_back((*(point(fv_it.handle()).data() + 1) - center[1]) / scalar);
			out_vertices.push_back((*(point(fv_it.handle()).data() + 2) - center[2]) / scalar);
	...
	std::cout << "Range: " << maxX << ", " << minX << ", " << maxY << ", " << minY << ", " << maxZ << ", " << minZ << endl;
	std::cout << "Center: " << center[0] << ", " << center[1] << ", " << center[2] << endl;
	modelCenter.push_back(center[0]);
	modelCenter.push_back(center[1]);
	modelCenter.push_back(center[2]);
